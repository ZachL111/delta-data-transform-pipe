...
`delta-data-transform-pipe` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies transform behavior through node-edge fixtures, with cycle and reachability reports and synthetic fixtures only.
...
The score is deterministic. The current threshold is 168. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 5.
